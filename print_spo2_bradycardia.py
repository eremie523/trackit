...
def test_details():
    ...
    # Load bradycardia_critical.csv
    ecg_values = []
    ppg_values = []
    with open(os.path.join('inference-service', 'demo_data', 'bradycardia_critical.csv'), 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            if not row: continue
    ...
    # We take the actual bradycardia section (from 30s to 75s)
    ecg_raw = np.array(ecg_values[30*256:60*256], dtype=np.float32)
    ppg_raw = np.array(ppg_values[30*64:60*64], dtype=np.float32)
